...
import { CreateSuccessDialog } from "./create-dialogs";
import {
...
export function EmployeeCreateWizard() {
  ...
  function goNext() {
    ...
    if (err) {
      addToast({ title: err, variant: "danger" });
      return;
    }
  ...
  async function submit() {
    ...
    try {
      ...
      const created = await api.post<{ id: string }>("/employees", {
        name: form.name.trim(),
        email: form.email.trim().toLowerCase(),
        phone: `+966${form.phoneLocal.replace(/\D/g, "")}`,
        ...
        emergencyContactPhone: form.emergencyContactPhone.trim()
          ? `+966${form.emergencyContactPhone.replace(/\D/g, "")}`
          : undefined,
      ...
      addToast({
        title: "خطأ",
        description: e instanceof Error ? e.message : "تعذر إنشاء الموظف",
        variant: "danger",
      });
    } finally {
      setSaving(false);
        ...
        <div className="grid gap-0 lg:grid-cols-[315px_1fr]">
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          done && !active && "bg-white/20 text-primary-foreground",
                          active && "border border-primary bg-card text-primary",
                          !done &&
                            !active &&
          ...
          <div className="flex min-h-[28rem] flex-col">
            <div className="flex-1 space-y-5 p-5 sm:p-6">
              {step === 1 ? (
                <StepBasic form={form} patch={patch} />
              ) : null}
              {step === 2 ? <StepPersonal form={form} patch={patch} /> : null}
              {step === 3 ? <StepJob form={form} patch={patch} /> : null}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="رقم الهاتف" required>
          <Input
            ...
            onChange={(e) => patch("phoneLocal", e.target.value)}
...
function StepJob({
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold">تفاصيل الوظيفة</h2>
        <p className="mt-1 text-sm text-muted-foreground">
      ...
      <div className="space-y-3 border-t border-border/70 pt-5">
        <h3 className="text-base font-bold">فترة الاختبار</h3>
...
function StepPayroll({
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold">إعدادات الرواتب</h2>
        <p className="mt-1 text-sm text-muted-foreground">
      ...
      <div className="space-y-3">
        <h3 className="text-base font-bold">المزايا</h3>
        <div className="grid gap-2 sm:grid-cols-2">
          {benefits.map((b) => {
            ...
            return (
              <label
                key={b.key}
                className="flex cursor-pointer items-center gap-3 rounded-xl border border-border/80 bg-card px-3 py-3"
              >
                <Checkbox
                  checked={checked}
                  onCheckedChange={(v) =>
      ...
      <div className="flex items-center gap-3">
        <div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-full bg-primary/15 text-lg font-bold text-primary">
          {form.photoUrl ? (
            ...
            form.name ? form.name.trim().split(/\s+/).slice(0, 2).map((p) => p[0]).join("") || "؟" : "؟"
          )}
        </div>
        <div>
