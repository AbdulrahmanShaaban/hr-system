...
export function TabSalaries({
  ...
  async function downloadSlip(id: string, month: number, year: number) {
    try {
      ...
      addToast({ title: "تم تحميل القسيمة", variant: "success" });
    } catch (err) {
      addToast({
        title: "خطأ",
        description: err instanceof Error ? err.message : "تعذر التحميل",
        variant: "danger",
      });
    }
  }
      ...
      <div className="overflow-hidden rounded-2xl border border-border/80 bg-card shadow-[0_1px_3px_rgb(0,0,0,0.04)]">
        <div className="flex flex-col gap-3 border-b border-border/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            <SelectTrigger
              className={cn(
                "h-9! w-full min-w-[9.5rem] rounded-lg border-primary/40 bg-card ps-2.5 sm:w-auto",
                "text-sm font-medium text-foreground shadow-none",
                "hover:bg-card focus-visible:border-primary focus-visible:ring-primary/20",
              )}
            >
